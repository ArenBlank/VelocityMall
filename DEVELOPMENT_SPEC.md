...
- `velocity-mall-admin-web` 已提供登录、商品、订单、秒杀活动、优惠券、评论、媒体和搜索索引管理页面。
...
- Admin 接口统一使用 `/api/v1/admin/**`、管理员 JWT 和 `X-Admin-Id`，当前阶段不默认引入 RBAC。
...
- 订单地址快照、后台发货、用户确认收货。
- 管理员登录、商品 SPU/SKU 管理、SKU 封面上传、订单发货、秒杀活动管理/预热、优惠券管理、评论管理、搜索索引重建。
...
- 管理后台前台已覆盖登录、商品、订单、秒杀活动、优惠券、评论、媒体资源和搜索索引运维。
...
- GitHub Actions 已包含 Maven Build、Full Chain E2E 与 `k6 Load & Chaos Tests (CI Mode)`；CI k6 smoke 使用一次性 seed、测试用户生成和 Redis 秒杀库存预热来防回归。
...
- **Phase 24 后台闭环**：商品、订单、秒杀活动、优惠券、评论、媒体和搜索索引能力已具备，后续重点是 Admin Web 到 Admin API 的端到端验收、失败提示一致性和必要时引入 RBAC。
- **分类管理后台**：当前前台已有分类树查询，后台分类新增/编辑/排序能力仍可单独规划。
- **数据迁移治理**：CI 和本地 seed 已成为关键资产，后续需要明确迁移脚本规范；是否正式引入 Flyway/Liquibase 放到 Phase 24 后半段评估。
- **履约物流深化**：真实物流轨迹、签收异常、售后工单、退货审核等。
- **真实支付渠道**：当前仍是 mock 支付，后续可接入真实支付回调验签和流水。
