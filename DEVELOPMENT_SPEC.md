...
### Phase 16：用户评价与互动体系（规划中，未实现）

目标：

- 建立商品评价发布、修改、删除与分页查询能力。
...
- 只有已完成或已支付订单的购买用户可以发布对应商品评价。
- 同一用户对同一订单明细或同一商品购买记录只能发布一条有效评价。
- 评价修改与删除必须校验评价归属，禁止用户操作他人评价。
- 评价列表支持按商品分页查询，并隐藏或脱敏非必要用户信息。
- 点赞和点踩必须具备幂等防重机制，同一用户对同一评价只能保持一种互动状态。
- 点赞数、点踩数等计数字段不得因重复请求或并发请求出现负数、错乱或重复累加。
- 高频查询优先考虑缓存评价聚合统计，评价正文列表以数据库分页查询为主，避免过早将复杂评论流系统化。
