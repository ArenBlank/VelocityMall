...
- 当前代码主干能力已经覆盖 `Phase 1` 到 `Phase 7` 的核心内容，并且已经补齐 `Phase 10` 到 `Phase 15`。
- `Phase 8/9` 当前未在代码主干中体现为独立落地模块或增量 SQL，可视为历史预留或已被后续规划覆盖。
- 旧文档曾将“用户评价与互动体系”写作 `Phase 8`，但代码尚未实现；为避免误导，后续统一建议按未来 `Phase 16` 或独立新增阶段规划。
- Gemini 后续规划不应再判断“项目只到 Phase 8”，而应以当前已实现的 Phase 10-15 为基线。
...
| Phase 16 用户评价与互动 | 发评/改评/删评、查看评价、点赞/点踩 | 规划中，代码未实现 |
...
2. 启动 product、order、seckill、search、coupon、gateway 六个应用。
3. 等待 Nacos 注册传播。
4. 验证分类树查询。
5. 验证 SPU 详情查询。
6. 验证 SKU 详情查询。
...
以下能力在文档或产品方向中已经出现，但当前代码尚未完整实现。注意：当前项目已经完成 Phase 10 到 Phase 15，后续规划应以这些能力为基线继续追加阶段，不要再按“当前停留在 Phase 8”理解：

- 用户评价与互动体系（建议作为未来 Phase 16 或独立 Review 服务阶段）：
  - 发评、改评、删评。
  - 商品评价分页查询。
  - 点赞、点踩。
  - 互动幂等、防重复操作、计数一致性。
...
建议 Gemini 在规划未来开发时遵循：

1. 优先不要破坏当前已跑通的 E2E 链路。
2. 不要把“Phase 8”理解为当前项目终点；当前代码已经实现到 Phase 15，用户评价建议作为 Phase 16 或后续独立阶段规划。
3. 新增用户评价功能时，建议作为独立 `velocity-mall-review` 服务或先在 product/order 边界内评估后再拆分。
4. 评价发布必须依赖订单购买记录，不允许任意用户评价任意商品。
5. 点赞/点踩必须建交互记录表，不能只更新计数字段。
6. 点赞和点踩应互斥，同一用户对同一评价只能存在一种有效态。
7. 高频展示统计可以缓存，评价正文列表先使用 MySQL 分页，避免过早引入复杂评论流系统。
8. 新增功能必须补充 E2E 验证：发评、改评、删评、查看评价、重复点赞、点赞切换点踩、非法操作他人评价。
9. 常规交易链路不要为了“万级 QPS”过度设计；项目当前定位是常规千级、热点读和秒杀入口数千级演示。
