...
| 管理后台前台 | 已新增 `velocity-mall-admin-web`，覆盖登录、RBAC 管理、按权限菜单/按钮控制、商品、订单、秒杀活动、优惠券、评论、媒体和搜索索引管理页面 |
...
| 数据库迁移 | 已接入 FlywayDB；公共迁移脚本位于 `velocity-mall-common/src/main/resources/db/migration`，本地演示数据位于 `db/devdata` 并默认不启用；`doc/sql` 仅保留为历史和手工参考 |
...
当前主干已经覆盖商品、购物车、订单、支付/退款 mock（含交易流水、模拟回调验签和幂等处理）、秒杀、数据库驱动秒杀活动、搜索、优惠券、评价、用户、地址、C 端买家前台、管理后台前台、后台 RBAC 权限校验与管理页面、后台发货、商品 SPU/SKU 管理、优惠券管理、秒杀活动管理、评论管理、搜索索引重建、MinIO 商品封面和基础 Trace 透传；性能侧已经完成 Phase 23 的 Gateway x3 与 Seckill x3 本地混合集群极限压测。
...
| `velocity-mall-admin` | `velocity-mall-admin` | 管理员登录、RBAC 权限校验、管理员/角色/授权管理、商品 SPU/SKU 管理、SKU 封面上传、订单发货、秒杀活动管理/预热、优惠券管理、评论管理、搜索索引重建 | 8089 |
| `velocity-mall-web` | 前端应用 | C 端买家前台：登录/注册、首页搜索、商品详情、秒杀抢购、排队结果、购物车、地址、优惠券、订单、mock 支付、评价 | 5173 |
| `velocity-mall-admin-web` | 前端应用 | 管理后台前台：登录、RBAC 管理、按权限菜单/按钮控制、商品、订单、秒杀活动、优惠券、评论、媒体资源、搜索索引运维 | 5174 |
...
后台 RBAC 在 `velocity-mall-admin` 服务内完成，Gateway 不查询角色权限表。Admin 服务会校验管理员是否启用，并按 `@RequireAdminPermission` 注解查询当前管理员权限码集合；无权限时返回业务码 `40300`。当前内置演示角色：

- `SUPER_ADMIN`：超级管理员，拥有全部后台权限。
- `OPS_STAFF`：运营账号，可维护商品、秒杀、优惠券、评价，可查看订单，不能发货或重建搜索索引。
- `VIEWER`：只读账号，只能查看商品、订单、秒杀、优惠券、评价。

RBAC 管理接口使用 `rbac:read`、`rbac:write` 权限码保护。超级管理员可在后台维护管理员账号、重置密码、启停账号、创建/编辑角色，并为角色分配后端内置权限码；`SUPER_ADMIN` 角色固定拥有全部权限且不允许禁用。
...
- `POST /api/v1/orders/pay/mock?orderSn=...&payType=1`：兼容前端的模拟支付入口，内部创建/复用支付流水并走模拟第三方回调验签和幂等处理。
- `POST /api/v1/orders/pay/mock/callback`：公开模拟支付回调入口，不要求用户 JWT，但必须通过 HMAC-SHA256 验签。
- `POST /api/v1/orders/{order-sn}/refund/mock`：兼容前端的模拟退款入口，内部创建/复用退款流水并走模拟第三方回调验签和幂等处理。
- `POST /api/v1/orders/refund/mock/callback`：公开模拟退款回调入口，不要求用户 JWT，但必须通过 HMAC-SHA256 验签。
...
- `POST /api/v1/admin/login`：管理员登录，返回 admin JWT、角色码和权限码。
- `GET /api/v1/admin/me`：查询当前管理员资料、角色码和权限码，用于前端刷新后恢复权限状态。
- `GET /api/v1/admin/rbac/admins`：分页查询后台管理员。
- `POST /api/v1/admin/rbac/admins`：新增后台管理员并绑定角色。
- `PUT /api/v1/admin/rbac/admins/{admin-id}`：编辑管理员资料、状态和角色绑定。
- `PUT /api/v1/admin/rbac/admins/{admin-id}/status`：启用或禁用管理员。
- `PUT /api/v1/admin/rbac/admins/{admin-id}/password`：重置管理员密码。
- `GET /api/v1/admin/rbac/roles`：查询角色列表及权限绑定。
- `POST /api/v1/admin/rbac/roles`：新增角色并绑定权限。
- `PUT /api/v1/admin/rbac/roles/{role-id}`：编辑角色和权限绑定。
- `PUT /api/v1/admin/rbac/roles/{role-id}/status`：启用或禁用角色。
- `GET /api/v1/admin/rbac/permissions`：查询后端内置权限码列表。
...
权限码当前覆盖 `product:read/write`、`order:read/deliver`、`seckill:read/write/preheat`、`coupon:read/write`、`review:read/delete`、`system:rebuild`、`rbac:read/write`。
...
- 权限恢复：登录响应和 `GET /api/v1/admin/me` 提供角色码、权限码，刷新页面后自动恢复权限状态。
- 权限管理：维护管理员、重置密码、启停账号、维护角色和分配权限码。
...
- 菜单、路由和关键按钮按权限码过滤；真正安全边界仍以后端 `@RequireAdminPermission` 校验为准。
- CI 已新增 `Admin Web Build`，固定执行 `npm ci` 与 `npm run build`，防止后台前端路由、类型和打包配置回归。
- 当前内置 `admin`、`operator`、`viewer` 三类演示账号，分别对应超级管理员、运营和只读角色。
...
Flyway 默认表结构迁移脚本位于 `velocity-mall-common/src/main/resources/db/migration/`；本地可选演示数据位于 `velocity-mall-common/src/main/resources/db/devdata/`。`doc/sql/` 仅保留历史 SQL、阶段增量脚本和人工排查参考，不再作为默认自动迁移入口。支付/退款流水表 `oms_payment_transaction` 由 `V2__create_payment_transaction.sql` 管理。
...
14. 验证 mock 支付、支付流水、模拟回调验签、重复支付/重复回调幂等、金额不一致拦截、订单状态、库存扣减、销量增加、消费幂等记录。
15. 验证管理员发货、物流公司/单号落库、用户确认收货。
16. 验证评价发布、评价列表、点赞、点赞切换点踩、匿名统计、删除评价。
17. 验证 mock 退款、退款流水、重复退款/重复回调幂等、库存回滚、销量回滚、消费幂等记录。
...
- 管理后台闭环：商品、订单、秒杀活动、优惠券、评论、媒体、搜索索引和 RBAC V1 能力已具备，后续重点是操作审计、端到端验收和错误提示一致性。
- 分类管理后台：当前前台已有分类树查询，后台分类新增/编辑/排序能力仍可单独规划。
- 支付系统：当前没有对接真实支付宝/微信渠道；mock 支付已具备交易流水、HMAC 回调验签、金额校验、订单状态条件更新和幂等处理，可作为后续真实渠道接入的骨架。
...
- 数据迁移治理：已引入 FlywayDB 统一管理默认表结构迁移；后续新增表、字段和索引应继续追加 `V*.sql`，本地演示数据保留为可选 repeatable migration。
