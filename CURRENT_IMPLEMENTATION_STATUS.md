...
> 同步基准：当前代码、配置、SQL、运行中 MySQL/MinIO 数据、`velocity-mall-web` 与 `scripts/ci/e2e.sh`。旧版 Markdown 如与本文件或代码冲突，以当前代码为准。
...
当前主干已经覆盖商品、购物车、订单、支付 mock、退款 mock、秒杀、数据库驱动秒杀活动、搜索、优惠券、评价、用户、地址、C 端买家前台、后台发货、商品上下架、MinIO 商品封面和基础 Trace 透传；性能侧已经完成 Phase 23 的 Gateway x3 与 Seckill x3 本地混合集群极限压测。
...
| `velocity-mall-admin` | `velocity-mall-admin` | 管理员登录、订单发货、商品上架/下架 | 8089 |
...
Phase 23 之后，项目在不改变总阶段编号的前提下补齐了 C 端买家前台、MinIO 商品封面兜底、数据库驱动的秒杀活动配置，以及“活动未开始”长倒计时演示商品。
...
- Nginx，宿主机入口 `80`，统一转发前端开发代理、Gateway 与 `/minio/` 静态资源。
...
- 管理后台完善：商品新增/编辑、分类管理、优惠券创建、活动配置等完整后台 API 仍可继续补齐。
...
2. 新增能力时按当前模块边界扩展，不要按旧文档把项目误判为停留在 Phase 3、Phase 8、Phase 16 或 Phase 21；当前总阶段是 Phase 23。
3. 涉及订单状态、库存、优惠券、评价互动、MQ 消费的改动，必须优先考虑条件更新、唯一约束和幂等。
4. 普通交易链路不要为了“万级 QPS”过度套用秒杀方案；秒杀链路继续保留 Redis Lua + MQ 削峰的专用设计。
